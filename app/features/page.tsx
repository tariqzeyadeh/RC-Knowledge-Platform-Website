...
import { LayoutGrid, ChevronDown, Target, Lightbulb, ShieldCheck, BadgeCheck } from "lucide-react"
import { AppShell } from "@/components/app-shell"
import { features, seciLayers } from "@/lib/data"
...
export default function FeaturesPage() {
  ...
  return (
    <AppShell
      title="دليل الميزات (30 ميزة)"
      description="الميزات الثلاثون للمنصة موزّعة على طبقات نموذج SECL لإدارة المعرفة، مع ربط كل ميزة بمعايير ISO 30401 ونموذج EFQM والنموذج الوطني للتميز."
      breadcrumb={[{ label: "الرئيسية", href: "/" }, { label: "دليل الميزات" }]}
    >
      {/* Layer filter */}
      <div className="mb-6 flex flex-wrap gap-2">
        <FilterChip label="جميع الميزات" count={features.length} active={layer === "all"} onClick={() => setLayer("all")} />
        {seciLayers.map((l) => (
              ...
              {isOpen && (
                <div className="border-t border-border px-4 pb-5 pt-4 sm:px-[72px]">
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-2">
                    <InfoBlock icon={<Target className="h-4 w-4" />} title="الأثر">{f.impact}</InfoBlock>
                    <InfoBlock icon={<Lightbulb className="h-4 w-4" />} title="مثال تطبيقي">{f.example}</InfoBlock>
                  </div>

                  <div className="mt-4">
                    <p className="mb-1.5 text-xs font-medium text-foreground">الأدوار المعنية</p>
                  ...
                  <div className="mt-4 grid gap-2 sm:grid-cols-3">
                    <Standard icon={<ShieldCheck className="h-3.5 w-3.5" />} label="ISO 30401" value={f.iso} />
                    <Standard icon={<BadgeCheck className="h-3.5 w-3.5" />} label="EFQM" value={f.efqm} />
                    <Standard icon={<BadgeCheck className="h-3.5 w-3.5" />} label="النموذج الوطني" value={f.national} />
                  </div>
                </div>
              )}
            </div>
