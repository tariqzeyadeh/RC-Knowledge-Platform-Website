...
import {
  Library, Search, Users, ClipboardCheck, ArrowLeft, TrendingUp, TrendingDown,
...
import {
  kpis, seciLayers, assets, categories, monthlyActivity, contentHealth, ORG,
} from "@/lib/data"
    ...
    <AppShell>
      ...
      <section className="animate-scale-in mb-8 overflow-hidden rounded-xl border border-border bg-primary text-primary-foreground">
        <div className="relative px-6 py-8 sm:px-10 sm:py-10">
          ...
          <div className="pointer-events-none absolute -left-8 top-1/2 h-32 w-32 -translate-y-1/2 rounded-full bg-gold/10 blur-3xl animate-float" aria-hidden="true" />
          <div className="relative max-w-2xl stagger-children">
            ...
            <p className="mt-3 max-w-xl text-sm leading-relaxed text-primary-foreground/80 text-pretty">
              منظومة موحّدة لتوثيق المعرفة المؤسسية ونقلها وتنظيمها واستخدامها وفق نموذج SECI، ومتوافقة مع
              معيار ISO 30401 والنموذج الوطني للتميز وضوابط الأمن السيبراني الوطنية.
            </p>
      ...
      <section className="stagger-children mb-10 grid grid-cols-2 gap-4 lg:grid-cols-4">
        {kpis.map((k) => {
          ...
          return (
            <Card key={k.label} className="interactive-card group">
      ...
      <section className="mb-10">
        <SectionHeading
          title="دورة المعرفة وفق نموذج SECI"
          action={<Link href="/features" className="link-arrow hover:underline">دليل الميزات <ArrowLeft className="h-3.5 w-3.5" /></Link>}
        />
        <div className="stagger-children grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {seciLayers.map((layer, i) => (
            <Card key={layer.id} className="interactive-card group relative overflow-hidden">
              <span className="absolute inset-y-0 right-0 w-1" style={{ background: layer.color }} aria-hidden="true" />
      ...
      <section className="stagger-children mb-10 grid gap-6 lg:grid-cols-5">
        <Card className="interactive-card lg:col-span-3">
          <CardHeader>
            <CardTitle className="font-heading text-base">نشاط المعرفة الشهري</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="interactive-card lg:col-span-2">
          <CardHeader>
            <CardTitle className="font-heading text-base">صحّة المحتوى المعرفي</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <section className="mb-10">
        <SectionHeading
          title="أحدث الأصول المعرفية"
          action={<Link href="/library" className="link-arrow hover:underline">عرض الكل <ArrowLeft className="h-3.5 w-3.5" /></Link>}
        />
      ...
      <section>
        <SectionHeading
          title="التصنيفات المعرفية"
          action={<Link href="/library" className="link-arrow hover:underline">المكتبة الكاملة <ArrowLeft className="h-3.5 w-3.5" /></Link>}
        />
            ...
            return (
              <Link key={c.id} href={`/library?cat=${c.id}`}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 text-xs text-muted-foreground">{c.count.toLocaleString("ar-SA")} أصل معرفي</p>
                </div>
              </Link>
            )
          })}
