    ...
    >
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {complianceMatrix.map((c) => {
          const statusKey = complianceStatusKey[c.status] ?? "compliant"
              ...
              <CardContent className="p-5">
                <div className="mb-3 flex items-start justify-between gap-2">
                  ...
                  <span className={cn("inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-medium",
                    ...
                    {isCompliant ? <CheckCircle2 className="h-3 w-3" /> : <Clock className="h-3 w-3" />}
                    {t(`enums.complianceStatus.${statusKey}`)}
                  </span>
                </div>
      ...
      <div className="mt-10 grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 font-heading text-base"><Lock className="h-5 w-5 text-primary" /> {t("pages.governance.securityControls")}</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="bg-primary text-primary-foreground">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 font-heading text-base text-primary-foreground">
              <Server className="h-5 w-5" /> {t("pages.governance.dataSovereignty")}
            </CardTitle>
          </CardHeader>
